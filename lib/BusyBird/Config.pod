...
=item C<default_status_storage> => L<BusyBird::StatusStorage> OBJECT

B<Default:> L<BusyBird::StatusStorage::Memory> object.
...
=item C<time_format> => STRFTIME_FORMAT_STR
...
=item C<post_button_url> => URL_STR

B<Default:> C<"https://twitter.com/intent/tweet">

Link URL attached to the "Post" button in the navigation bar.

=item C<default_status_permalink_builder> => CODEREF($status)

B<Default:> build permalink to twitter.com

Subroutine reference that is supposed to build permalink URL to the status.

If the status has the C<busybird.status_permalink> field, that field is used for the permalink.
Otherwise, C<default_status_permalink_builder> is used to build the permalink.

The builder subroutine reference is called with the status object, as in:

    $url = $builder->($status)

If the result C<$url> is C<undef> or does not look like a valid URL, it is ignored.

=item C<urls_entity_url_builder> => CODEREF($entity, $status)

B<Default:> return C<< $entity->{display_url} >>

=item C<media_entity_url_builder> => CODEREF($entity, $status)

B<Default:> return C<< $entity->{display_url} >>

=item C<user_mentions_entity_url_builder> => CODEREF($entity, $status)

B<Default:> build URL to the user page of twitter.com

=item C<hashtags_entity_url_builder> => CODEREF($entity, $status)
...
The builder subroutine reference is called with the twitter entity object and the status object, as in:

    $url = $builder->($entity, $status)

If the result C<$url> is C<undef> or does not look like a valid URL, it is ignored, i.e., the text is not linked.
