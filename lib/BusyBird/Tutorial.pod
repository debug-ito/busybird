...
Of course, you can use any CPAN client to install L<BusyBird> instead of getting cpanminus on the fly.
...
L<BusyBird> renders statuses based on their status "levels".
...
- make sample_config.psgi more friendly
- BusyBird is configured by ~/.busybird/config.psgi
- This is a Perl script.
- config is between "use BusyBird;" and "end;".

=head2 Configure More Than One Timelines

- Just put timeline("hoge") functions to create timelines.

=head2 Configuration Parameters

- busybird->set_config() and timeline(...)->set_config().
- details => Config.pod
- example: global time_zone setting and per-timeline time_zone setting.

=head2 Input Statuses from Inside BusyBird

- A bit advanced topic.
- You can do anything in config.psgi because it's Perl.
- timeline("...") returns Timeline, busybird() returns Main.
- embed import.pl to config.psgi
- caveat: to repeat loading, you need to fork anyway.

=head1 Filters

- pointer to L<BusyBird::Timeline>'s C<add_filter()> method.
