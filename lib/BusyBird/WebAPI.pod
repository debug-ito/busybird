...
C<is_success> attribute of the object is C<true>, and C<statuses> attribute is the array of status objects fetched.
...
In success, the HTTP response code is 200.
C<is_success> attribute of the response object is C<true> and
...
Watches updates in numbers of unacked statuses (i.e. unacked counts) from multiple timelines, and gets them when necessary.
